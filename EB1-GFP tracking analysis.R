# script to analyse EB1-GFP tracks collected from Trackmate
...
# EB1-GFP spinning disk
# 5 embryos, 1 pixel = 0.068 um, Zoom = 100x
# 0.6 seconds per frame
# What is the length and width of cell in um
# Obtain main track direction from tracks...and rotate image by deviation from main axis
# Trackmate parameters: object- 10 px, threshold -25: perfect circle for ROI
  # LAP tracker: Frame to frame linking:  5 px
               # Allow gap closing: Max distance - 5px; Max frame gap: 2
# Use spots in tracks stats file

# for work Mac
setwd("Desktop/OneDrive - sheffield.ac.uk/EB1-GFP movies_NB/Analysis/")

# for home Mac
setwd("/Users/valfred/Desktop/OneDrive - sheffield.ac.uk/EB1-GFP movies_NB/Analysis/") 
...
newtracks <- data.frame(read.csv("embr2_1.csv")) 
...
tracks_calc <- function (a, b){
  c <- within(merge(a,b,by="TRACK_ID"), {
    ...
    ANGLE_CAT_SINGLE <- cut(ANGLE_SINGLE, seq(0,180,4), include.lowest = TRUE)
    TIME_SINGLE <- ave(EDGE_TIME, TRACK_ID,
                       FUN=function(x) c(0, diff(x)))
    VELOCITY <- DISP_2/TIME_SINGLE
  })[,c("TRACK_ID","EDGE_TIME", "TIME","X_LOCATION", "Y_LOCATION",
        "DISP_X","DISP_Y", "DISP","DISP_TYPE", "ANGLE", "ANGLE_RAD", "X_SINGLE", "Y_SINGLE",
        "DISP_2", "ANGLE_SINGLE", "ANGLE_SINGLE_RAD", 
        "ANGLE_CAT_SINGLE", "TIME_SINGLE", "VELOCITY")]
...
#write.csv(tracks_norm, "e_norm.csv")
...
#####
# write.csv(tracks_norm, "embr6-1_norm.csv")
# write.csv(Y_POS_norm, "embr6-1_POS.csv")
# write.csv(Y_NEG_norm, "embr6-1_NEG.csv")
#######################################################
...
######
tracks_norm_POS <- as.data.frame(aggregate(tracks_summary$DISP_2 ~ tracks_summary$ANGLE_CAT_POS,
                                           tracks_summary, sum))
names(tracks_norm_POS) <- c("ANGLE_CAT", "DISP_SUM")
tracks_norm_POS$DISP_NORM <- tracks_norm_POS$DISP_SUM / sum(tracks_norm_POS$DISP_SUM)
sum(tracks_norm_POS$DISP_NORM)==1  # this should always be TRUE
...
tracks_norm_POS$ANGLE_CLASS <- midpoints(tracks_norm_POS$ANGLE_CAT)
tracks_norm_POS <- tracks_norm_POS[, c(1, 4, 2, 3)]
tracks_norm_POS <- tracks_norm_POS[order(tracks_norm_POS$ANGLE_CLASS),]

tracks_norm_NEG <- as.data.frame(aggregate(tracks_summary$DISP_2 ~ tracks_summary$ANGLE_CAT_NEG,
                                           tracks_summary, sum))
names(tracks_norm_NEG) <- c("ANGLE_CAT", "DISP_SUM")
tracks_norm_NEG$DISP_NORM <- tracks_norm_NEG$DISP_SUM / sum(tracks_norm_NEG$DISP_SUM)
sum(tracks_norm_NEG$DISP_NORM)==1  # this should always be TRUE
...
tracks_norm_NEG$ANGLE_CLASS <- midpoints(tracks_norm_NEG$ANGLE_CAT)
tracks_norm_NEG <- tracks_norm_NEG[, c(1, 4, 2, 3)]
tracks_norm_NEG <- tracks_norm_NEG[order(tracks_norm_NEG$ANGLE_CLASS),]
...
ggplot(na.omit(tracks_norm_POS), aes(x=ANGLE_CAT,y=DISP_NORM)) +  
  geom_bar(stat = "identity", width = 0.9, fill ="black", 
           col ="black", position="dodge") + theme_bw() +
  scale_x_discrete(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0), 
                     breaks = round(seq(0, max(tracks_norm_POS$DISP_NORM),
                                        by = 0.005),3)) +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust =1),
        axis.text.y = element_text(size = 10, vjust =1)) +
  xlab("Angle of Displacement (degrees)") +
...
ggplot(na.omit(tracks_norm_NEG), aes(x=ANGLE_CAT,y=DISP_NORM)) +  
  geom_bar(stat = "identity", width = 0.9, fill ="black", 
           col ="black", position="dodge") + theme_bw() +
  scale_x_discrete(expand = c(0, 0)) + 
  scale_y_continuous(expand = c(0, 0), 
                     breaks = round(seq(0, max(tracks_norm_NEG$DISP_NORM),
                                        by = 0.005),3)) +
  theme(axis.text.x = element_text(size = 10, angle = 45, hjust =1),
        axis.text.y = element_text(size = 10, vjust =1)) +
  xlab("Angle of Displacement (degrees)") +
